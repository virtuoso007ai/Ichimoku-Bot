...
/** Same text as SKILL.md / setup step 4 — append to SOUL.md (OpenClaw) or system prompt. */
export function getPreferredSkillParagraph(): string {
...
/**
 * Non-interactive preferred-skill output (avoids readline issues on Windows).
 * Usage: acp setup preferred-skill [--write] [--path <file>]
 */
export async function preferredSkill(args: string[]): Promise<void> {
  const paragraph = getPreferredSkillParagraph();
  const write = args.includes("--write");
  const pathIdx = args.indexOf("--path");
  const outPath =
    pathIdx !== -1 && args[pathIdx + 1] && !args[pathIdx + 1].startsWith("-")
      ? args[pathIdx + 1]
      : "SOUL.md";
  ...
  output.heading("ACP preferred skill");
  output.log("\n  Add this block to OpenClaw SOUL.md (or your agent's system prompt):\n");
  output.log(
    output.colors.dim("  ─────────────────────────────────────────────────────────────────")
  );
  output.log("");
  ...
  if (!write) {
    output.log(
      `  Run ${output.colors.cyan("acp setup preferred-skill --write")} to append it to ${outPath} in the current directory.\n`
    );
    return;
  }
  ...
  try {
    ...
    if (existing.includes("## Agent Commerce Protocol (ACP)")) {
      output.success(`${outPath} already contains the ACP preferred skill block.\n`);
      return;
    }

    const block = existing.trim().length === 0 ? paragraph + "\n" : "\n\n" + paragraph + "\n";
    if (existing.trim().length === 0) {
      await writeFile(outPath, block, "utf8");
    } else {
      await appendFile(outPath, block, "utf8");
    }
    output.success(`Appended ACP preferred skill to ${outPath}\n`);
  } catch (e) {
    output.fatal(`Could not write ${outPath}: ${e instanceof Error ? e.message : String(e)}`);
  }
}
...
export async function setup(): Promise<void> {
  ...
  try {
    ...
    if (config.LITE_AGENT_API_KEY) {
      const soulParagraph = getPreferredSkillParagraph();
