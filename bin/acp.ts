...
function buildHelp(): string {
  const lines = [
    ...
    cmd("setup preferred-skill", "Print or append ACP preferred skill (SOUL.md); flags: --write [--path file]"),
  ...
  const h: Record<string, () => string> = {
    setup: () =>
      [
        ...
        `    4. Optional ACP preferred skill (SOUL.md); or use subcommand below`,
        "",
        cmd("preferred-skill", "Print ACP preferred-skill block (no login)"),
        flag("--write", "Append block to SOUL.md (or --path)"),
        flag("--path <file>", "File to write (default: SOUL.md)"),
        "",
      ].join("\n"),
...
async function main(): Promise<void> {
  ...
  if (command === "setup") {
    const setupModule = await import("../src/commands/setup.js");
    if (subcommand === "preferred-skill") {
      return setupModule.preferredSkill(rest);
    }
